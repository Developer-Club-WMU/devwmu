...
function PublicEventsPage() {
  ...
  const renderEventCard = (event: typeof upcoming[0]) => (
    <Card 
      key={event.id} 
      className="group relative overflow-hidden rounded-none border-0 bg-public-card transition-all duration-500 hover:shadow-[0_0_40px_rgba(246,200,78,0.15)] backdrop-blur-sm h-full"
    >
      <Link to="/events/$eventId" params={{ eventId: event.id }} className="block h-full relative z-10 flex flex-col justify-between p-2">
        <div>
          <CardHeader className="relative z-10">
            <CardTitle className="text-2xl font-black uppercase text-white group-hover:-translate-y-1 transition-transform group-hover:text-public-accent duration-300">
              {event.title}
            </CardTitle>
            <CardDescription className="flex flex-col gap-1 mt-2 transition-all opacity-90 group-hover:opacity-100 group-hover:-translate-y-1 duration-300">
              <span className="font-medium text-public-accent">
